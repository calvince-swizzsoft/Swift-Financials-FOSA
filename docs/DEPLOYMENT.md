...
### Build, update and verify the Windows service

Follow [Windows service deployment and recovery](../tools/deployment/WINDOWS-SERVICE-DEPLOYMENT.md)
for the validated package builder, one-command updater, service identity/queue
permissions, and the `--check-startup` diagnostic. The registered service name is
`SwiftFinancialsService`.

The 2026-09-06 repair fixes a deployed Unity.Container 5.8.6 / required 5.8.13
mismatch and prevents startup failures from being hidden by a background task.
Several plugin HintPaths formerly pointed into the Unity 5.8.6 meta-package even
though their assembly references declared newer versions. Use the canonical
standalone packages and package verification instead of copying stale DLLs.

Do not assume a particular domain identifier from source notes: the domain in
queued messages, dispatcher `uniqueId`, and named database connection must agree
with the actual deployed API. The updater preserves server settings and applies
only the new runtime binding redirects.
