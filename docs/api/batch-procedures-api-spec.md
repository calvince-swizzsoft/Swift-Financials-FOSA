...
### Loan disbursement effective dates

The loan disbursement batch create DTO accepts `EffectiveDisbursementDate` (date-only, e.g. `2026-08-25`). It defaults to the server's current date and can be updated by its creator while the batch is Pending. The date is returned in batch reads and displayed during verification/authorization. `PUT /api/accounts/loandisbursementbatches/{id}` accepts EffectiveDisbursementDate alongside Reference and Priority. Date changes revalidate every attached loan and the posting period. Verification/authorization and updates lock the same batch row, preventing a date change from racing verification. Once verified, the date is locked.

AppServices validate the date at creation, entry attachment, verification, authorization and worker posting. It must not be future-dated, must fall within exactly one open/unlocked posting period, and must not precede the loan's ReceivedDate or approval. A saved insider board decision of `Approved` supplies the historical approval date; otherwise the loan's ApprovedDate is used. A later `Ratified` decision is not treated as an earlier approval. Validation failures return HTTP 409 `LOAN_DISBURSEMENT_DATE_INVALID` with an actionable message.

The posting worker uses the effective date for every disbursement/fee/offset journal, its corresponding posting period, LoanCase.DisbursedDate, and repayment/standing-order due dates. LoanCase.DisbursementProcessedDate records the actual processing time; existing CreatedDate/authorization/audit timestamps remain unchanged. Legacy batches without a date acquire today's date during authorization; already-authorized legacy batches use AuthorizedDate. Existing posted loans are not rewritten.

Insider Form 9 selection uses board/disbursement and appointment business dates, without excluding cases created later. The loan notice register likewise uses application/disbursement business dates. August reporting still requires August-effective insider tenure and confirmed schedule/evidence, and values deposits/ledger postings as at the relevant date.
