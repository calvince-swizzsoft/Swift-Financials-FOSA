    ...
    public class NavigationMenu
    {
        ...
        public List<NavigationMenu> GetMenus()
        {
            var menus = new List<NavigationMenu>()
            {
                ...
                // Per-type children (codes 70-78) deliberately commented out, not deleted -
                // permission granularity for Batch Procedures was dropped from "per type per
                // stage" down to "per stage" (see WebApplication1/Areas/Accounts/
                // BATCH-PROCEDURES-CONCEPTS.md). Roles are now granted against the three
                // stage-level parents above/below (codes 69/79/89) directly; the frontend
                // renders one screen per stage with all nine types as client-side tabs, not
                // gated individually. Left here, commented, in case per-type granularity is
                // ever wanted back - re-enabling is just uncommenting plus re-running the
                // SwiftFinancials.Utility seeder.
                ...
                // Per-type children (codes 80-88) commented out - see note above Batch Origination's children.
                ...
                // Per-type children (codes 90-98) commented out - see note above Batch Origination's children.
