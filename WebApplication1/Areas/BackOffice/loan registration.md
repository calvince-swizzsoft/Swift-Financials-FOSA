...
Use this module to register Loans/Advances into the system. At this stage, the details of the loan applicant, such as the type of loan applied, amount applied, guarantors etc will be entered into the system by the authorized officer.

To access this sub module click on:Loan Origination Module -> Navigate -> Operations -> Application - > Registration.
...
Note: Once you select the appropriate loan product, fields that are related to the loan will populate i.e Loan Section, Term (Months), Loan Balance if a member has a balance of the same loan and Investment Balance will also be shown automatically.

- In the "Amount Applied" field, enter the amount that the customer is applying for.
- For the ''Loan Purpose field; Click on the lookup icon to select the appropriate loan purpose.
- Enter the date received (the date that the loan application was made)

At the bottom at that screen we have three tabs namely:
...
In some cases customers do issue standing orders to the bank. Therefore to be able to see/view these instructions click on the "Standing Orders" Button.

All the standing orders instructions will be displayed.
...
For loans that requires one to have guarantors as part of the loan security, click on the guarantors button to capture guarantors.

Click on the customer look up icon in the "Customer" field, and select the relevant customer who is to be the guarantor.

Click on the Guarantors Button, to capture guarantors as shown in the figure below.

Click on the customer look up icon and select/search for the relevant customer who is to be the guarantor as shown below.
...
will be displayed respectively. Input the amount of shares to be guaranteed as the amount guaranteed.
...
If the customer applying for a loan has satisfied all the loan registration requirements, input the registration remarks as confirmed and click on the update button to proceed to the next phase of loaning.
...
Select the other remarks appropriately, i.e, if a customer has not provided their payslip, select no payslip.

Click on the update button to save changes. The loan application is automatically loaded into the Loan Cases File, where it will be shown as "Registered".
