...
Seven real bugs and one missing controller were found here and have since
...
| 5 | Correctness risk | Open | `ChequesController.ClearSelectedCheques` takes both a string `actionType` and a separate int `clearingOption` from the client and trusts them to stay in sync instead of deriving one from the other server-side. A mismatched pair (e.g. `actionType: "unpay"` + `clearingOption: 1`) silently takes the `Pay` branch — genuine wrong-money-movement risk, not a no-op, since a mismatched Pay branch on an eligible cheque still runs the real Pay logic. |
...
| 11 | Efficiency / redundancy | Open | `ChequesController.ClearSelectedCheques` calls `ExternalChequeAppService.ClearExternalCheque` — which already sets `persisted.IsCleared`/`ClearedBy`/`ClearedDate` and saves internally (`ExternalChequeAppService.cs:488-490`, `570-572`) — and then immediately calls `MarkExternalChequeCleared` again on success (`ChequesController.cs:194`, `228`), which sets the exact same three fields a second time via a second `dbContextScope`. Redundant, harmless (idempotent), extra DB round-trip per cheque. |
