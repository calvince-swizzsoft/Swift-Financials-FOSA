...
All nine types are now built (`docs/api/batch-procedures-api-spec.md` has
the full route reference for each). Audience: anyone maintaining or
extending `Areas/Accounts/Controllers/*BatchController.cs`, or wondering why
`NavigationMenu.cs` splits every batch type into three separate
"Origination / Verification / Authorization" menu groups.
...
|---|---|---|
| **Credit** | Crediting many member accounts from one source in one run. `Payout` = savings/dividend interest payout runs. `CheckOff` = an employer remits one lump sum of payroll deductions; the batch allocates it across many members' loan repayments, share contributions, welfare/insurance premiums, etc. — confirmed in `CreditBatchAppService`, which has explicit `CheckOffEntryType` handling per product line (`sLoan`, `sInterest`, `sShare`, `wCont`, `sInvest`, `sRisk`, `wLoan`, `sLoanInterest`). `CashPickup`/`SundryPayments` pay non-members. **Real bug found and fixed, found much later** than this controller was originally built (on a follow-up audit pass across all nine types, triggered by the `LoanDisbursementBatch`/`MarkLoanCaseDisbursed` bug — `Areas/BackOffice/WORKFLOW.md` §4/§11): `AuthorizeCreditBatch`'s precondition that the batch already be `Audited` was entirely commented out in source, replaced by a bare null check — a batch could be authorized straight from `Pending`, bypassing the maker-checker Audit step. Same category of control-bypass as the Inter Account Transfer fix below. Restored the real check. | `CreditBatchDTO` / `ICreditBatchAppService` — **built** (`CreditBatchController.cs`) |
...
| **Disbursement** | Releasing approved loan principal to members in bulk, after a loan case has cleared appraisal/approval/audit upstream (`Areas/BackOffice/WORKFLOW.md` §14.1-14.4). Interface also exposes `DisburseMicroLoan` and a transaction-threshold validator that look like a separate alternate-channel (USSD/API) disbursement path, not part of this batch UI — confirm with product before deciding whether those belong on the same controller. **Real bug found and fixed, found late** (while building the loan-case appraisal/approval/audit endpoints, not while this controller was originally built): `ILoanCaseAppService.MarkLoanCaseDisbursed` — called from `PostLoanDisbursementBatchEntry` after the disbursement journal already posts — only matched `case LoanCaseStatus.Approved:` in its switch, but a loan case that went through the intended pipeline is `Audited` (a distinct enum value) by the time it's disbursed. Silently returned `false` for every correctly-audited case, meaning the loan case never flipped to `Disbursed` and the repayment `StandingOrder` never got created, even though the money had already moved. Fixed to match `Audited`. | `LoanDisbursementBatchDTO` / `ILoanDisbursementBatchAppService` (lives in `BackOfficeModule`, but the reference app still routes its screens under Areas/Accounts) |
...
| **Inter Account Transfer** | One **source** customer account (the header) transfers its balance out to however many entries you attach, each targeting *either* a customer account or a raw G/L account (`ApportionTo`, genuinely consulted — unlike Voucher's lookalike dead fields). A `DynamicCharges` sub-resource attaches real transfer-fee tariffs, fed into posting. **Fixed a real control-bypass bug** while building: `AuthorizeInterAccountTransferBatch` used to force-set status to `Audited` *before* checking it was already `Audited` (and before null-checking), so the "must be Audited first" gate was tautologically always true — a batch could be authorized straight from `Pending`. Also flagged, not fixed: no control-total validation exists anywhere for this type — `AvailableBalance` has no backing column and is never re-verified server-side, so nothing stops an over-transfer. `Authorize` posts synchronously, one `Journal` per entry via the same `AddNewJournal` entry point the front office uses (not `BulkSave`). No `Priority` field, unlike every other type here. | `InterAccountTransferBatchDTO` / `IInterAccountTransferBatchAppService` — **built** (`InterAccountTransferBatchController.cs`) |
